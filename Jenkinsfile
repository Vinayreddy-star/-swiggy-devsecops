  ...
  stages {
    ...
    stage('Deploy') {
      steps {
sh '''
  docker rm -f $(docker ps -aq --filter ancestor=vinayreddy99/swiggy-devsecops) 2>/dev/null || true
docker run -d --name swiggy-app-${BUILD_NUMBER} --restart unless-stopped \\
    --memory=512m -p 3000:3000 -e PORT=3000 -e BUILD_NUMBER=${BUILD_NUMBER} \\
